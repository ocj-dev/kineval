...
/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\/||\
    ...
    Pendularm Dynamical Simulation | three.js Scene (ALTDRAW variant)

    An alternative, simplified drawing scene -- one vertical post and a
    crossbar rather than the upstream stencil's full 4-leg table -- used by
    pendularm_altdraw.html. The primary pendularm.html instead uses scene.js,
    a faithful port of the original kineval-stencil's own three.js rig.
    ...
    Each link swings by rotating about its group's LOCAL X AXIS (not Z): the
    crossbar and post both lie in the X-Y plane at z=0, so a swing that
    rotated about Z (also confined to the X-Y plane, with the pivot sitting
    right at the crossbar's own position) would sweep the rod directly
    through the crossbar/post every frame. Rotating about X instead sweeps
    the rod through the Y-Z plane -- X stays fixed at the pivot's own X
    position while Z varies -- moving it away from the stand's geometry as
    soon as the angle leaves zero.
...
// arrow. Rotating about the LOCAL X AXIS (see the file header) keeps the
// swing plane clear of the stand's crossbar/post, which both lie at z=0.
function updatePendulumMeshes(rig, pendulum) {
    ...
    for (var i = 0; i < rig.links.length; i++) {
        ...
        rig.links[i].group.rotation.x = relative;
    }
...
// Arrows point along +/-Z (the same axis the swing itself now sweeps
// through), matching updatePendulumMeshes()'s X-axis rotation above.
function updateArrow(arrow, origin, signedTorque) {
    ...
    arrow.setDirection(new THREE.Vector3(0, 0, signedTorque >= 0 ? 1 : -1));
