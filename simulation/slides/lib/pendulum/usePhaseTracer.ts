...
// (a swinging pendulum, a PID loop) run indefinitely rather than
// terminating -- this just stops an idle, endlessly-Play()ing tab from
// growing the trace array forever
const MAX_ENTRIES = 4000

// Generic play/pause/step/reset engine, carried over unchanged from the
// cloth_simulation deck's lib/cloth/usePhaseTracer.ts (same problem shape:
// a small physical system stepped through in phases). `generateFrame()`
// advances the caller's own physics state by exactly one simulation frame
// and returns that frame's phase snapshots (e.g. compute-error / pid /
// accelerate / integrate); this composable only owns the bookkeeping --
// growing the trace lazily, and navigating it -- not the physics itself.
//
