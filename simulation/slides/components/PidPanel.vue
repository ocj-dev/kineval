...
interface Entry { frame: number; phase: Phase; angle: number; error: number; control: number; errorSeries: number[] }

let angle = [RELEASE_ANGLE], angle_dot = [0]
...
function generateFrame(): Entry[] {
  ...
  const e1: Entry = { frame, phase: 'error', angle: angle[0], error, control: control[0], errorSeries: errorSeries.slice() }

  const servo = { kp: [gains.kp], kd: [gains.kd], ki: [gains.ki] }
  const pid = PID(angle, [desired.value], previous_error, accumulated_error, DT, servo)
  control = pid.control; previous_error = pid.previous_error; accumulated_error = pid.accumulated_error
  const e2: Entry = { frame, phase: 'pid', angle: angle[0], error, control: control[0], errorSeries: errorSeries.slice() }

  const accel = pendulumAcceleration(angle, angle_dot, control, GRAVITY, [MASS], [LENGTH])
  const e3: Entry = { frame, phase: 'accelerate', angle: angle[0], error, control: control[0], errorSeries: errorSeries.slice() }
  ...
  const e4: Entry = { frame, phase: 'integrate', angle: angle[0], error, control: control[0], errorSeries: errorSeries.slice() }
